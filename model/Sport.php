...
class Sport extends koneksi
{
    ...
    {
        $query = "SELECT * FROM sport ORDER BY id_sport DESC";
    ...
    {
        $sql = ("INSERT INTO sport(nama_sport, harga, rating, deskripsi, cover) 
                   VALUES(?,?,?,?,?)");
    ...
    {
        $sql = "UPDATE sport 
                SET nama_sport = ?, harga = ?, rating = ?, deskripsi = ?, cover = ?
                WHERE id_sport = ?";
        $query = $this->conn->prepare($sql);
    ...
    {
        $sql = "DELETE FROM sport WHERE id_sport = ?";
        $query = $this->conn->prepare($sql);
    ...
    {
        $sql = "SELECT * FROM sport WHERE id_sport = ?";
        $query = $this->conn->prepare($sql);
        $query->bind_param("i", $id);
        $query->execute();
