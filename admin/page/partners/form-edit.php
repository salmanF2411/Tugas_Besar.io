...
<body>
  <div class="container">
    <div class="card">
      <form action="page/partners/edit.php" method="post" enctype="multipart/form-data" id="furnitureForm">
        <h2 class="form-title">Edit Produk partner</h2>

        <input type="hidden" name="id_partner" value="<?= $id; ?>">
        <input type="hidden" name="cover_lama" value="<?= $data['cover']; ?>">

        <div class="form-grid">
          <label for="nama" class="form-label">Nama Partner</label>
          <input type="text" id="nama" name="nama_partner" class="form-input" value="<?= $data['nama_partner']; ?>" required />
        </div>
  ...
  <script>
    ...
    document.getElementById('furnitureForm').addEventListener('submit', function() {
      setTimeout(function() {
        window.location.href = '../../dashboard.php?module=partners&page=daftar-partners';
      }, 100);
    });
  </script>
</body>
