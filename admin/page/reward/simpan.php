    ...
    if (isset($_FILES['cover']) && $_FILES['cover']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $allowedExtensions)) {
            ...
            if (!move_uploaded_file($_FILES['cover']['tmp_name'], $coverPath)) {
                header('Location: ../../dashboard.php?module=reward&page=daftar-reward&pesan=gagal_upload');
                exit;
            }
        } else {
            header('Location: ../../dashboard.php?module=reward&page=daftar-reward&pesan=format_tidak_valid');
            exit;
        }
    }

    $reward = new Reward();
    $result = $reward->simpan($nama, $deskripsi, $coverName);
