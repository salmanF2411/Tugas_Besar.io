...
<body>
  <div class="container">
    <div class="card">
      <form action="page/reward/edit.php" method="post" enctype="multipart/form-data" id="furnitureForm">
        <h2 class="form-title">Edit Reward</h2>

        <input type="hidden" name="id_reward" value="<?= $id; ?>">
        <input type="hidden" name="cover_lama" value="<?= $data['cover']; ?>">

        <div class="form-grid">
          <label for="nama" class="form-label">Nama reward</label>
          <input type="text" id="nama" name="nama_reward" class="form-input" value="<?= $data['nama_reward']; ?>" required />
        </div>
  ...
  <script>
    ...
    document.getElementById('furnitureForm').addEventListener('submit', function() {
      setTimeout(function() {
        window.location.href = '../../dashboard.php?module=reward&page=daftar-reward';
      }, 100);
    });
  </script>
</body>
