    ...
    if (isset($_FILES['cover']) && $_FILES['cover']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $allowedExtensions)) {
            ...
            if (!move_uploaded_file($_FILES['cover']['tmp_name'], $coverPath)) {
                header('Location: ../../dashboard.php?module=metode-pembayaran&page=daftar-mpembayaran&pesan=gagal_upload');
                exit;
            }
        } else {
            header('Location: ../../dashboard.php?module=metode-pembayaran&page=daftar-mpembayaran&pesan=format_tidak_valid');
            exit;
        }
    }

    $pembayaran = new Mpembayaran();
    $result = $pembayaran->simpan($nama, $deskripsi, $coverName);
