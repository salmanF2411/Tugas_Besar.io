    ...
    if (isset($_FILES['cover']) && $_FILES['cover']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $allowedExtensions)) {
            ...
            if (!move_uploaded_file($_FILES['cover']['tmp_name'], $coverPath)) {
                header('Location: ../../dashboard.php?module=produk-elektronik&page=felektronikpesan=gagal_upload');
                exit;
            }
        } else {
            header('Location: ../../dashboard.php?module=produk-elektronik&page=elektronik&pesan=format_tidak_valid');
            exit;
        }
    }

    $elektronik = new Elektronik();
    $result = $elektronik->simpan($nama, $harga, $rating, $deskripsi, $coverName);
