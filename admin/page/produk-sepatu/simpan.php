    ...
    if (isset($_FILES['cover']) && $_FILES['cover']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $allowedExtensions)) {
            ...
            if (!move_uploaded_file($_FILES['cover']['tmp_name'], $coverPath)) {
                header('Location: ../../dashboard.php?module=produk-sepatu&page=sepatu&pesan=gagal_upload');
                exit;
            }
        } else {
            header('Location: ../../dashboard.php?module=produk-sepatu&page=sepatu&pesan=format_tidak_valid');
            exit;
        }
    }

    $sepatu = new Sepatu();
    $result = $sepatu->simpan($nama, $harga, $rating, $deskripsi, $coverName);
