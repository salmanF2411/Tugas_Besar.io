...
<body>
  <div class="container">
    <div class="card">
      <form action="page/produk-furniture/edit.php" method="post" enctype="multipart/form-data" id="furnitureForm">
        <h2 class="form-title">Edit Produk Furniture</h2>

        <input type="hidden" name="id_furniture" value="<?= $id; ?>">
        <input type="hidden" name="cover_lama" value="<?= $data['cover']; ?>">

        <div class="form-grid">
          <label for="nama" class="form-label">Nama Furniture</label>
          <input type="text" id="nama" name="nama_furniture" class="form-input" value="<?= $data['nama_furniture']; ?>" required />
        </div>
  ...
  <script>
    ...
    document.getElementById('furnitureForm').addEventListener('submit', function() {
      setTimeout(function() {
        window.location.href = '../../dashboard.php?module=produk-furniture&page=furniture';
      }, 100);
    });
  </script>
</body>
